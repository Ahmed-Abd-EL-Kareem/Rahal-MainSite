...
export default function EditTripPage() {
  ...
  if (fetching) {
    return (
      <main className="flex-1 w-full bg-bg text-ink1 pt-24 sm:pt-28 pb-16 sm:pb-20 px-4 sm:px-6 md:px-8 max-w-[860px] mx-auto flex flex-col items-center justify-center gap-3 min-h-[60vh]">
        <Loader2 className="w-8 h-8 animate-spin text-brand" />
        <span className="text-sm text-ink2 text-center">
    ...
    >
      ...
      <div className="mb-8 sm:mb-10">
        <button
          ...
          className="flex items-center gap-1.5 text-xs font-semibold text-ink2 hover:text-ink1 transition-colors border-none bg-transparent cursor-pointer mb-4"
        >
      ...
      <div className="flex flex-col gap-5 sm:gap-6">
        ...
        <div className="card bg-surface1 border border-border p-4 sm:p-6 flex flex-col gap-4">
          ...
          <Field label={locale === 'ar' ? 'رابط الصورة (اختياري)' : 'Image URL (optional)'}>
            <div className="relative">
              <Image size={14} className="absolute top-1/2 -translate-y-1/2 start-3 text-ink3" />
              <input type="url" value={imageUrl} onChange={(e) => setImageUrl(e.target.value)}
                placeholder="https://..." className="input w-full bg-surface2 ps-8" />
            </div>
          </Field>
        ...
        <div className="flex flex-col sm:flex-row gap-3 pb-2">
          <button
            onClick={() => router.push(`/trips/${tripId}`)}
            className="px-5 py-4 rounded-xl border border-border bg-surface1 text-ink2 hover:text-ink1 hover:border-border-glow transition-all text-sm font-semibold cursor-pointer w-full sm:w-auto order-2 sm:order-1"
          >
          ...
          <button
            ...
            className={cn(
              'btn-primary flex-1 py-4 flex items-center justify-center gap-2 text-base font-bold order-1 sm:order-2',
