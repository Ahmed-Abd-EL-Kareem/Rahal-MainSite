  ...
  return (
    <main className="min-h-screen flex flex-col md:flex-row bg-background">
      ...
      <section className="hidden md:flex relative md:w-1/2 lg:w-3/5 overflow-hidden bg-obsidian">
        <div 
          className="absolute inset-0 w-full h-full bg-cover bg-center transition-transform duration-10000 hover:scale-110 opacity-70" 
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-black/40 to-black/85 flex flex-col justify-between p-12 lg:p-20 text-white z-10">
          <div>
            <Link href="/" className="flex items-center gap-3">
              ...
              <span className="font-display font-bold text-headline-md text-primary-fixed-dim tracking-tight">Rahal</span>
            </Link>
          </div>

          <div className="max-w-lg space-y-8">
            <h1 className="font-display text-4xl lg:text-5xl font-bold leading-tight">
              {locale === 'ar' ? 'ابدأ رحلتك الأبدية' : 'Begin Your Eternal Journey'}
            </h1>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white/10 backdrop-blur-md p-6 rounded-xl flex items-start gap-4 border border-white/15">
                ...
                <div>
                  <h3 className="font-display text-lg font-semibold mb-1">
                    {locale === 'ar' ? 'مسارات ذكاء اصطناعي مخصصة' : 'Personalized AI Itineraries'}
                  </h3>
              ...
              <div className="bg-white/10 backdrop-blur-md p-6 rounded-xl flex items-start gap-4 border border-white/15">
                ...
                <div>
                  <h3 className="font-display text-lg font-semibold mb-1">
                    {locale === 'ar' ? 'حجز فوري ذكي' : 'One-Click Secure Bookings'}
                  </h3>
                  <p className="font-body text-sm text-surface-variant/90 leading-relaxed">
                    {locale === 'ar' 
      ...
      <section className="flex-1 bg-surface flex flex-col items-center justify-center p-6 md:p-12 lg:p-24 relative overflow-y-auto h-screen">
        ...
        <div className="md:hidden flex items-center gap-2 mb-8">
          ...
          <span className="font-display font-bold text-headline-sm text-primary">Rahal</span>
        </div>
        ...
        <div className="w-full max-w-md">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="space-y-1">
              <label className="text-xs font-semibold text-on-surface-variant block uppercase tracking-wider" htmlFor="name">
                {t('nameLabel')}
              </label>
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1 pt-1">
                ...
                <div className="w-full bg-surface-container-highest rounded-full flex gap-1 h-1">
                  <div className={`w-1/4 h-full rounded-full transition-all duration-300 ${passwordStrength >= 1 ? getStrengthColor() : 'bg-surface-container-high'}`}></div>
                  <div className={`w-1/4 h-full rounded-full transition-all duration-300 ${passwordStrength >= 2 ? getStrengthColor() : 'bg-surface-container-high'}`}></div>
                  <div className={`w-1/4 h-full rounded-full transition-all duration-300 ${passwordStrength >= 3 ? getStrengthColor() : 'bg-surface-container-high'}`}></div>
                  <div className={`w-1/4 h-full rounded-full transition-all duration-300 ${passwordStrength >= 4 ? getStrengthColor() : 'bg-surface-container-high'}`}></div>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <label className="text-xs text-on-surface-variant font-medium cursor-pointer" htmlFor="terms">
                {locale === 'ar' ? (
                  <>
                    أنا أوافق على <Link className="text-primary hover:underline font-bold" href="#">شروط الخدمة</Link> و <Link className="text-primary hover:underline font-bold" href="#">سياسة الخصوصية</Link>.
                  </>
                ) : (
                  <>
                    I agree to the <Link className="text-primary hover:underline font-bold" href="#">Terms of Service</Link> and <Link className="text-primary hover:underline font-bold" href="#">Privacy Policy</Link>.
                  </>
                )}
              </label>
            </div>
            ...
            >
              {loading ? (
                <span className="flex items-center justify-center gap-2">
                  ...
                  <span>{locale === 'ar' ? 'جاري التحضير...' : 'Creating...'}</span>
                </span>
              ) : (
          ...
          <button 
            ...
            className="w-full py-3 border border-outline-variant/50 hover:border-primary bg-white text-on-surface font-semibold text-sm rounded-xl flex items-center justify-center gap-3 transition-all hover:bg-surface-container-lowest"
          >
