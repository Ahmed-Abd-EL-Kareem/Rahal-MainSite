        ...
        <div className="flex-1 min-w-0 w-full">
          {trip.summary && (
            <div className="bg-surface border border-outline-variant/25 rounded-2xl shadow-sm p-4 sm:p-5 mb-6">
              <p className="text-sm text-on-surface-variant leading-relaxed">{trip.summary}</p>
            </div>
          )}
                ...
                >
                  <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-sm font-bold text-on-surface truncate">{day.title}</p>
                    </div>
                  </div>
                  ...
                  <div className="px-4 sm:px-5 pb-5 flex flex-col gap-4 border-t border-outline-variant/20">
                      ...
                      <div>
                        ...
                        <ul className="flex flex-col gap-1">
                          {day.meals.map((meal, i) => (
                            <li key={i} className="text-xs text-on-surface-variant">{meal}</li>
                          ))}
                        </ul>
                      </div>
                    )}
                    ...
                    {day.accommodation && (
                      <div>
                        ...
                        <p className="text-xs text-on-surface-variant">{day.accommodation}</p>
                      </div>
                    )}
                    ...
                    {day.tips && (
                      <div className="bg-tertiary/5 border border-tertiary/15 rounded-lg p-3">
                        ...
                        <p className="text-xs text-on-surface-variant">{day.tips}</p>
                      </div>
                    )}
                  </div>
                )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-3 text-sm">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <span className="text-on-surface font-semibold truncate block">{trip.destination}</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <span className="text-on-surface font-semibold capitalize">{trip.budget}</span>
                </div>
              </div>
            </div>
              ...
              <div className="border-t border-outline-variant/20 pt-4">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {trip.interests.map((i) => (
                    <span key={i} className="text-[10px] px-2 py-1 bg-surface-container border border-outline-variant/25 rounded-full text-on-surface-variant capitalize whitespace-nowrap">
                      {i}
                    </span>
                  ))}
                </div>
